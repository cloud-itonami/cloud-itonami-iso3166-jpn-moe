(ns greenprocurement.operation
  "OperationActor -- one MOE compliance operation = one supervised actor
  run, expressed as a langgraph-clj StateGraph. The advisor
  (GreenProcurement-LLM) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the Green Procurement Compliance
  ...
  One graph run = one compliance operation (green-purchasing track or
  eco-conscious-contracts track). No unbounded inner loop.
  ...
  governor is clean -- see `greenprocurement.phase`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [greenprocurement.greenprocurementllm :as greenprocurementllm]
            [greenprocurement.governor :as governor]
            [greenprocurement.phase :as phase]
...
(defn build
  ...
  [store & [{:keys [advisor checkpointer]
             :or   {advisor      (greenprocurementllm/mock-advisor)
  ...
  (-> (g/state-graph
      ...
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (greenprocurementllm/-advise advisor store request)]
            {:proposal p :audit [(greenprocurementllm/trace request p)]})))
