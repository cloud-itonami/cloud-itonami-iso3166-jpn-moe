(ns greenprocurement.registry
  "Pure-function filing-draft + filing-submit record construction for
  the JPN-MOE (Ministry of the Environment) actor -- an append-only
  compliance book-of-record draft, one per track (`:green-purchasing` /
  `:eco-conscious-contracts`).

  Like every sibling actor's registry, there is no single reference-
  number standard MOE assigns to a compliance filing-draft/filing-
  ...
  non-fabricating discipline `greenprocurement.facts` uses.
  ...
  call to MOE or any government portal. It builds the RECORD an
...
(defn register-draft
  ...
  (`:green-purchasing` or `:eco-conscious-contracts`) -- the operator's
  own act of preparing a MOE compliance filing package. Pure function
  -- does not touch MOE or any government portal."
  ...
  (let [draft-number (str "JPN-MOE-" (track-code track) "-DFT-" (zero-pad sequence 6))
...
(defn register-submit
  ...
  compliance filing to MOE (always human-gated upstream)."
  [engagement-id track sequence]
  (when-not (and engagement-id (not= engagement-id ""))
  ...
  (let [submit-number (str "JPN-MOE-" (track-code track) "-SUB-" (zero-pad sequence 6))
