...
(defn- tmpfile [suffix]
  (path/join (os/tmpdir) (str "moe-mutation-" suffix ".edn")))
...
(def with-pages
  ...
  branches."
  (subset #{"host.www-env-go-jp" "host.www-nies-go-jp"
            "law.green-purchasing" "env.home" "nies.home" "env.green-purchasing"}))
...
(def structural
  [{:id "coverage-entry-count"
   ...
   {:id "law-entry-without-law-id"
    ...
    :mutate #(alter-entity % "law.green-purchasing" (fn [e] (dissoc e :egov/law-id)))
...
(def network
  [{:id "nonexistent-law" :on :laws-only
    ...
    :mutate #(alter-entity % "law.green-purchasing" (fn [e] (assoc e :egov/law-id "999ZZ9999999999")))
    :want-exit 1 :want-reason "nonexistent-law"}

   {:id "law-title-drift" :on :laws-only
    :why "a real id recorded under the wrong title"
    :mutate #(alter-entity % "law.green-purchasing" (fn [e] (assoc e :egov/law-title "外国為替法")))
    :want-exit 1 :want-reason "law-title-drift"}

   {:id "law-num-drift" :on :laws-only
    :why "a real id recorded under the wrong law number"
    :mutate #(alter-entity % "law.green-purchasing" (fn [e] (assoc e :egov/law-num "昭和二十四年法律第二百二十九号")))
   ...
   {:id "fail-outranks-refusal" :on :laws-only
    ...
    :mutate (fn [d]
              (recount
                (-> d
                    (alter-entity "law.green-purchasing"
   ...
   {:id "page-title-drift"
    ...
    :mutate #(alter-entity % "nies.home" (fn [e] (assoc e :page/title "環境省")))
    :want-exit 1 :want-reason "title-drift"}

   {:id "missing-text"
    :why "a string the page does not contain"
    :mutate #(alter-entity % "env.green-purchasing"
                           (fn [e] (assoc e :page/must-contain ["第8次グリーン購入法基本方針"])))
